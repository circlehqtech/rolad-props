...
function ChartShell({ eyebrow, title, description, icon, children }: ChartShellProps) {
  return (
    <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 max-w-lg text-[11px] leading-5 text-slate-500">{description}</p>
        </div>
...
export function LiveMetricBars({
  ...
  return (
    <ChartShell eyebrow={eyebrow} title={title} description={description} icon={icon}>
      {loading ? (
          ...
          {cleanData.map((item, index) => (
            <div key={item.label}>
              <div className="mb-2 flex items-center justify-between gap-4 text-[11px]">
                <span className="truncate font-semibold text-slate-600" title={item.label}>
                  {item.label}
                </span>
              ...
              <div className="h-3 overflow-hidden rounded-full bg-slate-100">
                <div
                  ...
                  style={{
                    ...
                    backgroundColor: item.color || ["#0b909c", "#ff7758", "#64c977", "#c5a880"][index % 4],
                  }}
                />
              </div>
            </div>
...
export function LiveDonutChart({
  ...
  const palette = ["#0b909c", "#ff7758", "#64c977", "#c5a880", "#34515b"];
  const cleanData = data.filter((item) => Number.isFinite(item.value) && item.value >= 0);
        ...
        <div className="mt-6 grid grid-cols-1 items-center gap-6 sm:grid-cols-[170px_1fr]">
          ...
          <div className="space-y-2.5">
            {cleanData.map((item, index) => {
              ...
              return (
                <div key={item.label} className="flex items-center justify-between gap-3 rounded-lg bg-slate-50 px-3 py-2.5">
                  <div className="flex min-w-0 items-center gap-2">
                    <span className="h-2.5 w-2.5 shrink-0 rounded-full" style={{ backgroundColor: color }} />
                    <span className="truncate text-[11px] font-semibold text-slate-600">{item.label}</span>
                  </div>
                  <span className="shrink-0 text-[11px] font-bold tabular-nums text-slate-900">
                    {item.displayValue ?? item.value.toLocaleString()} <span className="text-slate-400">· {percentage}%</span>
                  </span>
                </div>
              );
            })}
  ...
  return (
    <div className="mt-6 grid min-h-44 place-items-center rounded-xl border border-dashed border-slate-200 bg-slate-50 text-center">
      <div>
        ...
        <p className="mt-2 text-[11px] font-semibold text-slate-500">Live chart data is not available yet.</p>
      </div>
    </div>
  );
}
